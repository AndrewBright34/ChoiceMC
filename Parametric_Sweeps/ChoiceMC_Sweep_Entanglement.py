...
for ig, g in enumerate(g_sweep):
    ...
    PIMC = ChoiceMC(m_max=5, P=9, g=g, MC_steps=100000, N=N, PIGS=True, Nskip=100, Nequilibrate=100)
    ...
    entanglement[ig,:] = [g, PIMC.S2, PIMC.S2_binning, PIMC.S2_err]
    entanglement_out.write(str(g) + ' ' + str(PIMC.S2) +  ' ' + str(PIMC.S2_binning) +  ' ' + str(PIMC.S2_err) + '\n')
...
S2_ax.errorbar(entanglement[:,0], entanglement[:,2], entanglement[:,3], fmt='.-', capsize=3, label='binning')
S2_ax.plot(entanglement[:,0], entanglement[:,1], label='original')
S2_ax.legend()
S2_ax.minorticks_on()
S2_ax.set_xlabel('g')
